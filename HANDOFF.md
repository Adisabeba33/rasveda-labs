...
| **Word Laboratory** | ✅ | `brands/word-laboratory.html` | ✅ Repo **`name-laboratory`** (v0.41.0): pure-TS meaning-first engine (single `analyzeMeaning()` seam, seeded mulberry32 RNG, **121** vitest tests green), React 18 · Vite 5 client, **5** Vercel functions `/api/analyze` · `/api/meanings` · `/api/usage` · `/api/semantic-search` · `/api/collision` (Anthropic SDK, Haiku 4.5 default, server-only key, graceful fallback). Engine has grown a lot since the page was first written — semantic gap search, breed-and-select lexical evolution (honest census), rule-based adoption test, semantic phonology/acoustics, morphological word families, imagined etymology, navigable semantic network, language refusals, target-type ranking, on-device lexicon, a "name something" brand mode, and a live key-free collision check. Page **updated this session** to match. Honesty rules on the page match the repo's own | "Site under development" |

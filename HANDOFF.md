...
| **Sianelara** (engine codename *Word Laboratory*) | ✅ | `brands/sianelara.html` · `styles/sianelara.css` | ✅ Repo **`name-laboratory`** (v0.47.0). **Rebranded to Sianelara — Institute of Meaning** (champagne-gold glyph mark on near-black; "Word Laboratory" is now the internal engine codename only). Portfolio page + card **renamed and rethemed to gold this session** (`assets/sianelara-mark.png` from the repo's real logo). Under the hood: pure-TS meaning-first engine (single `analyzeMeaning()` seam, seeded mulberry32 RNG, **121** vitest tests green), React 18 · Vite 5 client, **5** Vercel functions `/api/analyze` · `/api/meanings` · `/api/usage` · `/api/semantic-search` · `/api/collision` (Anthropic SDK, Haiku 4.5 default, server-only key, graceful fallback). Engine grown: semantic gap search, breed-and-select lexical evolution (honest census), rule-based adoption test, semantic phonology/acoustics, morphological word families, imagined etymology, navigable semantic network, language refusals, target-type ranking, on-device lexicon, a "name something" brand mode, and a live key-free collision check. Honesty rules on the page match the repo's own | "Site under development" |
